...
function createNameFromEmail(email: string) {
  const emailName = email.split("@")[0] || "Öğrenci";
...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    localStorage.setItem("subjeva-display-name", displayName);

    const selectedRole = localStorage.getItem("subjeva-selected-role");
